...
def vis_one_loc(img_path, words, loc_scores, bbox_pred, bbox_gt, module_names,
    ...
    T = cfg.MODEL.T_CTRL     ##########set to 12- bssically the modules used for the program
...
def vis_one_stepwise(img_path, words, module_names, txt_att, att_stack,
    ...
    T = cfg.MODEL.T_CTRL
    #ipdb.set_trace()           ## ##########set to 12- bssically the modules used for the program
    # M = len(module_names)
    img = skimage.io.imread(img_path)
    ...
    for t in range(T):
        ...
        # output attention                 #################this is what is something of our interest- visualizing attention
        #show_ans = 1      #edit_vedika
        if show_ans:
            if vis_type == 'vqa':
                ...
                #ipdb.set_trace()
                plt.axis('off')
                # if cfg.TEST.VIS_SHOW_GT_ANSWER :              ## CHANGES_VEDIKA
                #     answer_txt = (
                #         'predicted answer: "%s"\ntrue answer: "%s"' % (
                #             answers[np.argmax(vqa_scores)], answers[label]))
                # else:
                #     answer_txt = (
                #         'predicted answer: "%s"\ntrue answer: not given in test set' % (
                #             answers[np.argmax(vqa_scores)]))
            ...
            plt.subplot((T+2)*2, 3, (6*t_disp+9, 6*t_disp+12))
            att = np.sum(att_stack[t] * stack_ptr[t], axis=-1)           ###########ATTENTION and Stack!!
            #ipdb.set_trace()
            img_with_att = attention_interpolation(img, att)           ###########see what is this attn - vedika
            plt.imshow(img_with_att)
            plt.xticks([], [])
            plt.yticks([], [])
        plt.title('%s(%s)\n' % (m_display, att_txt), fontsize=24)
        patches = Arrow(                                                                         ########aplots arrow of certian width from x to x+dx, y to y+dy
            img.shape[1] // 2, -35, 0, 32, width=40, color='r', clip_on=False)              ############give better color here?    #comment_vedika   ####k is black; originally k
    ...
    print('predicted answer: "%s"\ntrue answer: "%s"' % (                                       ### changed by vedika: printing these things- btw you want to visulaize the true answer
                            answers[np.argmax(vqa_scores)], answers[label]))
...
def vis_batch_vqa(model, data_reader, batch, vis_outputs, start_idx,
    ...
    for n in inds:
        ...
        if not(cfg.TEST.VIS_SHOW_GT_ANSWER):
            label =0
        else:
                label = batch['answer_label_batch'][n]       ##comment when test2015
        #label = 2377                                ## vedika hack- handled by adding cfg.TEST.SHOW_GT_ANSWER
        txt_att = vis_outputs['txt_att'][n]               #################goldmine
        att_stack = vis_outputs['att_stack'][n]               #################goldmine
        stack_ptr = vis_outputs['stack_ptr'][n]                    #################goldmine
        module_prob = vis_outputs['module_prob'][n]
        if cfg.TEST.STEPWISE_VIS:
            vis_one_stepwise(img_path, words, module_names, txt_att, att_stack,                 # the new stepwise visualiztion- set to ture in config file
...
def _att_softmax_2(att):                                 ###########vedika version
    exps = np.exp(att)             ##########not deleting the max thing here- visualize how things differ
    softmax = exps / np.sum(exps)
    #ipdb.set_trace()
    return softmax
...
def attention_interpolation(im, att):
    #softmax = _att_softmax(att)               ### original
    softmax = _att_softmax_2(att)             ####edit_vedika    ###another goldmine kind
    ...
    # make the attention area brighter than the rest of the area                ###########GOLDMINE____-attention to be made brighter- just make it super bright
    #vis_im = att_reshaped * im + (1-att_reshaped) * im * .45   ###original        ## the first part is attention area masked- next part for background- so we want that dark- low values
    vis_im = att_reshaped * im + (1 - att_reshaped) * im * 0.0
    #ipdb.set_trace()
