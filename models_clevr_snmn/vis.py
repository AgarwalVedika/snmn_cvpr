...
def _find_txt_segs(keep, words):
    ...
    if elems:
        segs.append('"' + ' '.join(elems) + '"')
        #ipdb.set_trace()             segs is the keywords list_ c_t list  ['"what"', '"big"', '"?"']
    return segs


def _extract_txt_att(words, atts, thresh=0.5):
    """
    Take at most 3 words that have at least 50% of the max attention.                 ######## c_t: atmost 3 words
    """
    ...
    keep = (atts >= att_min)
    # assert np.any(keep)
    vis_txt = ', '.join(_find_txt_segs(keep, words))                          ### words- in the question
    #ipdb.set_trace()            #print(vis_txt)                               ### np.size(_find_txt_segs(keep, words))- gives the number of c_t
    num_txt = np.size(_find_txt_segs(keep, words))
    return vis_txt, num_txt
...
def vis_one_stepwise(img_path, words, module_names, txt_att, att_stack,
    ...
    T = cfg.MODEL.T_CTRL
    # M = len(module_names)
    #print('MODULE LIST LENGTH IS ', len(module_names))
    img = skimage.io.imread(img_path)
    ...
    #print(m_list)  # ['_Find', '_Transform', '_DescribeOne', '_NoOp', '_NoOp', '_NoOp', '_NoOp', '_NoOp', '_NoOp']                        #module_list
    ### comment_Vedika:  is len(m_list) a good representative of the program length?  yes for now- discuss with Mario- no; ypu need all (m_t, c_t) to get the program length
    ...
    for t in range(T):
        ...
        if m in {'_Scene', '_NoOp', '_And', '_Or'}:
            ...
            att_txt, num_txt = _extract_txt_att(words, txt_att[t, :len(words)])                 #edit_vedika textual_count

        if t == 0 and m == '_Filter':
            m_display = 'Filter/find'
        else:
            m_display = m[1:].replace(
                'Find', 'Find/ look_for').replace(
                'Filter', 'Filter/ select').replace(
                'Transform', 'Transform/ related_by').replace(
                'DescribeOne', 'DescribeOne/ Answer').replace(
                'DescribeTwo', 'DescribeTwo/ Compare_Two').replace(
                'And', 'And/ Intersect').replace('Or', 'Or/ Combine').lower()
        ...
        if show_ans:
            if vis_type == 'vqa':
                ...
                plt.text(10, 100, answer_txt, fontsize=20)                                                        #predicted answer being plotted
            elif vis_type == 'loc':
                plt.subplot((T+2)*2, 3, (6*t_disp+9, 6*t_disp+12))
                plt.imshow(img)
            ...
            plt.subplot((T+2)*2, 3, (6*t_disp+9, 6*t_disp+12))
            att = np.sum(att_stack[t] * stack_ptr[t], axis=-1)           ### attention stack- stack pointer involved
            img_with_att = attention_interpolation(img, att)
            plt.imshow(img_with_att)
            plt.xticks([], [])
            plt.yticks([], [])
        plt.title('%s(%s)\n' % (m_display, att_txt), fontsize=24)                                # (m_display, att_txt)
        #print(t, m_display,att_txt)
        #ipdb.set_trace()
        patches = Arrow(
            img.shape[1] // 2, -35, 0, 32, width=40, color='k', clip_on=False)
        plt.gca().add_patch(patches)
        t_disp += 1
        module_num += 1                                                                 #module_num and t_disp ae same
        text_count += num_txt
        #print('in line 348- vis.py program length_stepwise', module_num+text_count)
        pred_m_display.append('[' + m[1:] + ' (' + att_txt + ')' +']')
        #print('stack pointer count', t_disp)    segs.append('"' + ' '.join(elems) + '"')
        #print(pred_m_display)
    pred_m_display.append( 'pred_prog_len: ' + str(module_num+text_count))
    print('pred_prog_len: ', module_num+text_count)
    print('pred_prog: ', pred_m_display)
    num_count = ('pred_prog_len: "%s"')
    #plt.text(10, 100, answer_txt, fontsize=20)
    plt.text(10,130, pred_m_display, fontsize=20)
...
def vis_batch_vqa(model, data_reader, batch, vis_outputs, start_idx,
    ...
    if cfg.TEST.VIS_SEPARATE_CORRECTNESS:
        ...
        #ipdb.set_trace()                # print(predictions)
        ...
        inds = range(num)
        # print(num, inds)  num=64; inds= range(0,64)
    for n in inds:
        ...
        if cfg.TEST.STEPWISE_VIS:
            vis_one_stepwise(img_path, words, module_names, txt_att, att_stack,
                             ...
                             answers=answers)
            pred_layout_length =  vis_one_stepwise(img_path, words, module_names, txt_att, att_stack,
                             stack_ptr, module_prob, save_path, vis_type='vqa',
                             vqa_scores=vqa_scores, label=label,
                             answers=answers)
            #ipdb.set_trace()  # print(img_path, module_names, txt_att, att_stack,stack_ptr, module_prob, save_path, vqa_scores,label,answers)
        else:
