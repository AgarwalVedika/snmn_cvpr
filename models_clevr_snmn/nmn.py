...
class NMN:
    def __init__(self, kb_batch, c_list, module_names, module_prob_list,
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            for t in range(self.T_ctrl):
                ...
                # stack underflow or overflow. e.g. _Filter can't be run at                                           ###### vedi_comment SMART!- no stack overflow
                ...
                #ipdb.set_trace()   # print len(self.att_stack_list.append(att_stack_avg)).....
                att_stack_prev = att_stack_avg
                stack_ptr_prev = stack_ptr_avg
                mem_prev = mem_avg
                #print(np.shape(self.att_stack_list))
                #print(np.shape(self.stack_ptr_list))
                #print(np.shape(self.mem_list))
    ...
    def Scene(self, att_stack, stack_ptr, mem_in, c_i, scope='Scene',
              reuse=None):
        """
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            att_out = _1x1conv(
                'conv_att_out', tf.nn.l2_normalize(self.kb_batch, axis=-1),
                output_dim=1)
