...
class Controller:
    ...
    def __init__(self, lstm_seq, q_encoding, embed_seq, seq_length_batch,
        ...
        T_ctrl = cfg.MODEL.T_CTRL                          ### number of modules
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            for t in range(T_ctrl):
                ...
                if cfg.MODEL.CTRL.LINEAR_MODULE_WEIGHTS:
                    ...
                    # weights
                    module_logit = fc(
                        'fc_module_w_linear', cq_i, output_dim=num_module,
                        bias_term=False, reuse=(t > 0))  # [N, M]
                else:
                    # Apply a fully connected network on top of cq_i to predict
                    # the module weights
                    bias_term = cfg.MODEL.CTRL.MLP_MODULE_WEIGHTS_BIAS_TERM
                    module_w_l1 = fc_elu(
                        'fc_module_w_layer1', cq_i, output_dim=dim,
                        reuse=(t > 0))
                    module_logit = fc(
                        'fc_module_w_layer2', module_w_l1,
                        output_dim=num_module, bias_term=bias_term,
                        reuse=(t > 0))  # [N, M]
                ...
                #ipdb.set_trace()  ## get size fo this list
        ...
        #ipdb.set_trace()     ## get c_list and all