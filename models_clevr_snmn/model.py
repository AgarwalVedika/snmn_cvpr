...
class Model:
    def __init__(self, input_seq_batch, seq_length_batch, image_feat_batch,
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            # Controller and NMN
            num_module = len(module_names)           #set to 9 num_module
            #print('in model.py', num_module)
            self.num_module = num_module
            self.controller = controller.Controller(
                lstm_seq, q_encoding, embed_seq, seq_length_batch, num_module)
            self.c_list = self.controller.c_list
            #print('c list', self.c_list)      # [<tf.Tensor 'model/controller/Sum_1:0' shape=(?, 300) dtype=float32>, <tf.Tensor 'model/controller/Sum_3:0' shape=(?, 300) dtype=float32>, <tf.Tensor 'model/controller/Sum_5:0' shape=(?, 300) dtype=float32>, <tf.Tensor 'model/controller/Sum_7:0' shape=(?, 300) dtype=float32>, <tf.Tensor 'model/controller/Sum_9:0' shape=(?, 300) dtype=float32>, <tf.Tensor 'model/controller/Sum_11:0' shape=(?, 300) dtype=float32>, <tf.Tensor 'model/controller/Sum_13:0' shape=(?, 300) dtype=float32>, <tf.Tensor 'model/controller/Sum_15:0' shape=(?, 300) dtype=float32>, <tf.Tensor 'model/controller/Sum_17:0' shape=(?, 300) dtype=float32>]
            ...
            #ipdb.set_trace()        # visulaize lstm_seq, q_encoding, embed_seq, seq_length_batch, num_module
            ###  PRINT AND SEE THIS SELF.NMN LIST
            ...
            #ipdb.set_trace()         #visualize txt_att, att_stack. stack_ptr, module_prob and their length
