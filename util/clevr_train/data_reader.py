...
class DataReader1:
    def __init__(self, imdb_file, num_train_samples, shuffle=True, one_pass=False, prefetch_num=8,
                 **kwargs):
        print('Loading imdb from %s' % imdb_file)
        if imdb_file.endswith('.npy'):
            imdb = np.load(imdb_file)
            imdb = imdb[0:num_train_samples]
            ipdb.set_trace()
        else:
        ...
        ipdb.set_trace()   # print layout_dcit   AttributeError: 'BatchLoaderClevr' object has no attribute 'layout_dict'
